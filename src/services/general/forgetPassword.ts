...
export const forgetPasswordService = async (userData: forgetPasswordDto) => {
  const searchRedis = await getTokenCode(userData.email).catch((errorGetToken) => {
    console.log(errorGetToken);
    throw new Error(ERROR_MESSAGE.FORGET_PASSWORD_REDIS_ERROR);
  });

  if (searchRedis.code === userData.code) {
    const hashedPassword = await hashPassword(userData.password_user).catch((errorHashPassword) => {
    ...
    await forgetPasswordRepository.forgetPassword(userData).catch((errorForgetPassword) => {
