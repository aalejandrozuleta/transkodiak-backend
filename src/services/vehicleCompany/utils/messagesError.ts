export const ERROR_MESSAGE = {
  EXISTING: 'Este usuario ya existe',
  HASH_PASSWORD_FAILED: 'Error al hashear la contraseña',
  DB_ERROR: 'Error en la base de datos',
  CREDENTIALS: 'Credenciales incorrectas',
  BLOCKED_USER: 'El usuario está bloqueado',
  NOT_FOUND: 'La compañía vehicular no fue encontrada',
  DUPLICATE_NIT: 'El NIT de la empresa ya está en uso.',
  DUPLICATE_ENTRY: 'Uno de los datos ya existe en la base de datos.',
  DUPLICATE_EMAIL: 'El correo electrónico ya está en uso.',
  DUPLICATE_NAME: 'El nombre de la empresa ya está en uso.',
  DUPLICATE_PHONE: 'El numero de celular ya está en uso.',
  INHABILITE_USER: 'El transportador no se logra inhabilitar',
  EXISTING_EMAIL: 'El email ya esta en uso',
  EXISTING_NAME: 'El name de empresa ya esta en uso',
  SEND_EMAIL_FAILED: 'Error al enviar el correo'
};
