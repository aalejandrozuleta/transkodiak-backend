...
export const getInformationTransporterService = async (
  idTransporter: string,
) => {
  try {
    const [travelResult]: [getInformationTransporter[], FieldPacket[]] =
      await getInformationTransporterRepository
        .getInformation(idTransporter)
        .catch((error) => {
          console.error(error);
